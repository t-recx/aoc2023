...
impl Day for Day4 {
	fn one(&self, input: &str) -> String {
		return input
			...
			.fold(0, |acc, line| {
				...
				acc + if common_count == 0 { 0 } else { 2u32.pow(common_count - 1) }
			})
			.to_string();
	}
	...
	fn two(&self, input: &str) -> String {
		...
		return get_card_number(&lines, 0, lines.len(), &mut HashMap::new(), &mut HashMap::new()).to_string();
	}
}

fn get_card_number(lines: &[&str], start: usize, end: usize, common_counts: &mut HashMap<usize, u32>, card_numbers_per_indexes: &mut HashMap<(usize, usize), u32>) -> u32 {
	...
	while i < end {
		...
		if common_count > 0 {
			...
			if card_numbers_per_indexes.contains_key(&(new_start, new_end)) {
				...
				let card_number_indexes = get_card_number(lines, new_start, new_end, common_counts, card_numbers_per_indexes);
...
fn get_common_count(line: &str) -> u32 {
	let sides: Vec<Vec<i32>> = line
		...
		.collect::<Vec<&str>>()
		.iter()
		.map(|&x| x.split(' ').filter(|&x| !x.trim().is_empty()).map(|x| x.parse::<i32>().unwrap()).collect())
		.collect();
