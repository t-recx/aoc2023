            ...
            for internal_mappings in &almanac.mappings {
                for internal_mapping in internal_mappings {
                    if in_evaluation >= internal_mapping.source_range_start
                        && in_evaluation
                            <= internal_mapping.source_range_start + internal_mapping.range_length
                    {
                        in_evaluation = internal_mapping.destination_range_start
                            + (in_evaluation - internal_mapping.source_range_start);
                        break;
                    }
                }
            }
