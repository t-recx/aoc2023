...
impl Day for Day7 {
    fn one(&self, input: &str) -> String {
        return compute(
            input,
            get_number_of_cards_by_type_part_one,
            compare_hands_part_one,
        );
    }
    ...
    fn two(&self, input: &str) -> String {
        return compute(
            input,
            get_number_of_cards_by_type_part_two,
            compare_hands_part_two,
        );
    }
}
...
fn compute(
    input: &str,
    number_of_cards_by_type_fn: fn(&Vec<char>) -> HashMap<char, i32>,
    compare_hands_fn: fn(a: &Hand, b: &Hand) -> Ordering,
) -> String {
...
fn get_number_of_cards_by_type_part_two(chars: &Vec<char>) -> HashMap<char, i32> {
    ...
    let mut jokerless_card_counts: HashMap<char, i32> = char_counts
        ...
        .filter(|&(key, _)| key != 'J')
        .collect();
    let jokers_count: usize =
        chars.len() - jokerless_card_counts.values().cloned().sum::<i32>() as usize;
    ...
    if jokers_count > 0 {
        if jokers_count == 5 {
            ...
            if hand_type == 7 || hand_type == 6 {
                jokerless_card_counts
                    .entry(*jokerless_card_counts.iter().next().unwrap().0)
                    .and_modify(|v| *v += jokers_count as i32);
            } else if hand_type == 4 {
                let key = jokerless_card_counts
                    .iter()
                    .find(|&(_, v)| *v == 3)
                    .unwrap()
                    .0;
                ...
                let mut card_counts_vec: Vec<_> =
                    jokerless_card_counts.clone().into_iter().collect();

                card_counts_vec.sort_by(|(k1, _), (k2, _)| card_values[k2].cmp(&card_values[k1]));

                jokerless_card_counts
                    .entry(card_counts_vec[0].0)
                    .and_modify(|v| *v += jokers_count as i32);
            }
        }
...
fn get_type(number_of_cards_by_type: HashMap<char, i32>) -> i32 {
    if number_of_cards_by_type.len() == 1 {
        ...
        return 6;
    } else if number_of_cards_by_type
        .values()
        .filter(|&&value| value == 3)
        .count()
        == 1
        && number_of_cards_by_type
            .values()
