...
impl Day for Day8 {
    fn one(&self, input: &str) -> String {
        ...
        loop {
            let node_id =
                node[instructions.directions[step % instructions.directions.len()]].to_string();
    ...
    fn two(&self, input: &str) -> String {
        let instructions = get_instructions(input);
        let mut nodes_ids: Vec<String> = instructions
            .nodes
            .keys()
            .into_iter()
...
fn get_instructions(input: &str) -> Instructions {
    ...
    let streamlined_input: String = input
        .chars()
        .filter(|&x| x != '(' && x != ')' && x != ' ')
        .collect();
    ...
    for line in streamlined_input.lines() {
        if i == 0 {
            ...
            nodes.insert(
                tokens[0].clone(),
                vec![tokens_nodes[0].clone(), tokens_nodes[1].clone()],
            );
        }
