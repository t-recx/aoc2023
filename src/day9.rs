...
impl Day for Day9 {
	fn one(&self, input: &str) -> String {
		...
		return histories.iter().flat_map(|x| x.iter().flat_map(|y| y.last())).sum::<i32>().to_string();
	}

	fn two(&self, input: &str) -> String {
		let histories = get_histories(input);

		return histories.iter()
			.map(|x| x.iter()	
				.map(|y| y.first())
				.rev()
...
fn get_histories(input: &str) -> Vec<Vec<Vec<i32>>> {
	...
	for line in input.lines() {
		...
		sequences.push(line.split_whitespace().filter_map(|x| x.parse().ok()).collect());
